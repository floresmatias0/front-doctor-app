...
const DoctorCalendar = ({ currentDateState, setDaySelected, calendarEvents, daySelected, reserveTime, reserveTimeFrom, reserveTimeUntil, reserveTimeFrom2, reserveTimeUntil2 }) => {
  ...
  const generateEventsForDay = (day) => {
    const eventDivs = [];
    const startTime1 = setHours(day, reserveTimeFrom || 9);
    const endTime1 = setHours(day, reserveTimeUntil || 18);
    const startTime2 = reserveTimeFrom2 !== null ? setHours(day, reserveTimeFrom2) : null;
    const endTime2 = reserveTimeUntil2 !== null ? setHours(day, reserveTimeUntil2) : null;
    const now = new Date();
    ...
    while (currentTime < (endTime2 || endTime1)) {
        ...
        const eventEndTime = addMinutes(currentTime, reserveTime);
        ...
        if (isEventAvailable || (isToday(day) && eventStartTime <= now)) {
            emptyEvent = false;
        }
                ...
                <Button
                    ...
                    onClick={() => {
                        if (emptyEvent) {
                            ...
                            setDaySelected(format(eventStartTime, 'yyyy-MM-dd HH:mm')); // Formatear la fecha como cadena
                        }
                    }}
                    border="1px"
        ...
        currentTime = addMinutes(currentTime, reserveTime || 30); 
    }
    return eventDivs;
  };
  ...
  useEffect(() => {
    ...
    setMapEvents(events);
  }, [currentDateState, reserveTime, reserveTimeFrom, reserveTimeUntil, reserveTimeFrom2, reserveTimeUntil2, calendarEvents]);
